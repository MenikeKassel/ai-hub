...
## 0. 当前快照（2026-09-26 01:20 · 本机复核）

| 项 | 现状 |
|---|---|
| 控制台 8123 | ✅ `/api/ready` 正常，托管任务 `KOL_UI_Start` 重启成功（LastResult=0） |
| 审核窗口 | 9/26 今日队列 23 篇 new、0 pending、0 failed；下一晨报 0 pending；当前有 1 条候选待 AI，9:00 交付仍为 pending |
| X 采集 | 3 个槽位的限流冷却均已结束；修复状态投影后 3/3 显示 ready+enabled。9/25 晚批 83/83 被旧前置检查拦截；修复后尚未进行真实抓取验收，跨批待处理队列为 722 queued + 5 cooldown |
| 知乎采集 | 56 个活跃账号中最近 6 个失败；9/25 晚间任务因市场锁争用退出 1，原始帖子写入仍需按运行记录核对 |
...
| O-7 | **晚间知乎采集与行情锁争用** | 9/25 19:20 知乎任务启动，19:30 行情发布任务同时启动；19:34 `kol-post-fetch` 在股票线索对账处等待 `.market.lock` 超时，计划任务结果 1。需要错开调度或使线索对账延后执行；采集本身的成功/失败以 `fetch_runs` 为准。 |

控制台、行情和收益主链运行；O-6、O-7 影响采集与分类完整性。
...
| R-25 | **X 冷却过期后前置检查仍挡住全部账号** | `slots()` 以前只把过期 cooldown 显示为 ready，未同步计算有效 enabled；采集前置检查因此误判 0 个可用槽位。现在只有冷却到期且身份、凭据齐全时才投影 ready+enabled，原始冷却审计保留。 | 9/26；定向回归通过；控制台重启后 3/3 ready+enabled，真实抓取待下一次受控任务验收 |
...
| # | 项 | 说明 / 关注点 |
|---|---|---|
| W-1 | **X 采集限流与缺口** | 9/26 冷却过期后的本地资格已恢复，尚未做真实抓取验收；跨批队列 722 queued + 5 cooldown。纪律：auth / 限流 / 供应商 / 本地预算四态区分，**不绕限流换源**；X 与知乎分母分开报。**红线：采集凭据不用主账号** |
