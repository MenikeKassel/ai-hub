...
| O-7 | **计划任务安装态仍与仓库契约不一致** | 只读 `kol-task-doctor.ps1` 检出知乎晚间仍为 19:20（契约 18:00）、07:00 任务权限不同、08:45 任务缺 `-NoFetch`。08:45 运行脚本已有时窗兜底；19:00 后采集会延后股票线索对账，CLI 的持久重放表防止锁超时丢线索。当前账户无计划任务修改权限，提权启动被自动审批审查拦截；需管理员应用契约。 |
...
| R-28 | **多个安装脚本反复覆盖同名计划任务** | KOL 采集与晨间任务改为 `kol-task-contract.json` 单一契约，由 `install-kol-recovery-tasks.ps1` 注册；旧 post-fetch 安装器委托该入口，research-data 安装器不再注册或删除这些任务。只读 doctor 比较契约与实际 Windows 任务，并仅告警 X 高预算覆盖。 | 9/26；静态所有权测试、PowerShell 语法与本机 doctor 验收；在线任务仍需管理员应用 |
...
- **2026-09-26 17:35**：将 KOL 采集与晨间计划任务收敛到单一契约和注册入口；market/research 安装器不再覆盖或删除 KOL 任务。只读 doctor 检出本机三项安装态差异，保留管理员应用动作。
