...
| R-29 | **X 补抓单页 20 条误判 50 条目标失败** | 晨间首页页不再结算历史补抓；带游标的历史页累加深度，存在下一页时继续排队。首页采集保留历史游标，历史页不覆盖最新帖子 ID；重试按原目标重置旧游标。晨报明确标识 09:00 交付快照，档案可选数据码改为中文说明。 | 9/26；69 项采集回归、界面测试及生产构建通过；既有 `needs_review` 行需按需重新排队 |
...
- **2026-09-26 23:15**：修复 X 历史补抓把首页 20 条误判为 50 条不足，以及历史游标被首页覆盖的问题；控制台已重启。19:00 批次在 50/83 触发真实限流，冷却后单次续跑新增 31 个成功账号、233 篇帖子，批次累计 81/83；剩余 `honcia13`、`cyrilxuq` 为 NotFound 身份缺口。09:00 晨报 102/140 保留为当时的交付审计快照；既有 51 条 X `needs_review` 补抓状态不批量重排，避免再次触发真实限流。
