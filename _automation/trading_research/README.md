...
hypotheses generated by DeepSeek V4 Flash through the OpenCode Go API. Every
DeepSeek-backed job uses the Go chat-completions endpoint. Credentials are read
from Windows Credential Manager first and an environment variable or OpenCode
environment reference second; credentials are never logged or returned by the
API. Codex is not an
