...
class HermesOperatorContractTests(unittest.TestCase):
    def test_manual_collection_is_ai_independent(self) -> None:
        ...
        canonical_installer = (ROOT / "scripts" / "install-kol-recovery-tasks.ps1").read_text(encoding="utf-8")

        self.assertIn("KOL_Post_Fetch_Manual_X", operator)
        self.assertIn("KOL_Post_Fetch_Manual_Zhihu", operator)
        self.assertIn("-CollectionOnly", installer)
        self.assertIn("-SkipAiPrefill", canonical_installer)
        self.assertIn("-NotifyOnCompletion", canonical_installer)
