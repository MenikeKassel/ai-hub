...
export default function System() {
  ...
  return <section>
    ...
    <div className="health-grid">
      ...
      <HealthItem icon={Terminal} label="X session pool" ok={h?.x_collection_status === 'ready'} value={`${h?.x_collection_status || 'pending_verification'} / ${h?.x_sessions?.limit_mode === 'unlimited' ? `${h?.x_sessions?.global_used_24h ?? 0} requests audited · unlimited` : `${h?.x_sessions?.global_remaining_24h ?? 0} global requests left`}`} />
      <HealthItem icon={Terminal} label="X public backup" ok={!!h?.public_backup?.enabled && !h?.public_backup?.paused_until} value={`${h?.public_backup?.enabled ? 'enabled' : 'disabled'} / ${h?.public_backup?.limit_mode === 'unlimited' || h?.public_backup?.public_limit_mode === 'unlimited' ? `${h?.public_backup?.public_used_24h ?? 0} requests audited · unlimited` : `${h?.public_backup?.public_remaining_24h ?? 0} public requests left`}`} />
    ...
    <div className="panel scope-note"><strong>X 采集保护</strong><span>{h?.x_sessions?.limit_mode === 'unlimited' ? '主源无限制（已取消日额度和请求间隔）' : '三个会话共用滚动额度；轮换不会提高总额度'}；真实429、认证挑战仍会暂停。</span><button className="secondary-button" onClick={() => patchXPolicy.mutate(!Boolean(h?.x_sessions?.paused_until))}>{h?.x_sessions?.paused_until ? '恢复自动采集' : '暂停 X 自动采集'}</button><span>公开单帖备用：{h?.public_backup?.limit_mode === 'unlimited' || h?.public_backup?.public_limit_mode === 'unlimited' ? '无限制' : h?.public_backup?.paused_until ? `暂停至 ${h.public_backup.paused_until}` : h?.public_backup?.enabled ? '普通故障可用' : '已停用'}。</span><button className="secondary-button" onClick={() => patchPublicBackup.mutate(h?.public_backup?.enabled ? { paused: !Boolean(h.public_backup.paused_until) } : { enabled: true })}>{h?.public_backup?.enabled && !h.public_backup?.paused_until ? '暂停公开备用' : '启用公开备用'}</button></div>
  </section>
}
...
function XSessionForm({ slot, values, busy, onChange, onSave, onVerify, onToggle }: { slot: XSessionSlot; values: CredentialValues; busy: boolean; onChange: (value: CredentialValues) => void; onSave: () => void; onVerify: () => void; onToggle: () => void }) {
  ...
  return <form className="panel credential-form" onSubmit={(event) => { event.preventDefault(); onSave() }}>
    ...
    <small>{slot.limit_mode === 'unlimited' ? `已用 ${slot.used_24h ?? 0}（仅审计，无额度上限）` : `已用 ${slot.used_24h ?? 0}/${slot.session_limit_24h ?? 90}（滚动 24 小时）`}{slot.user_id ? ` · X ID ${slot.user_id}` : ''}{slot.cooldown_until ? ` · 冷却至 ${slot.cooldown_until}` : ''}</small>
