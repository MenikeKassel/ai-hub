...
export default function System() {
  ...
  return <section>
    ...
    <div className="health-grid">
      ...
      <HealthItem icon={Terminal} label="DeepSeek backup AI" ok={!!h?.deepseek_credentials_configured} value={`${h?.deepseek_model || 'deepseek-v4-flash'} / ${h?.deepseek_credentials_configured ? 'configured' : 'missing key'}`} />
    ...
    <div className="content-grid system-grid credential-stack-grid">
      ...
      <form className="panel credential-form" onSubmit={(event) => { event.preventDefault(); saveDeepSeek.mutate() }}>
        <div className="panel-heading"><div><h2><KeyRound size={16} />DeepSeek 备用 AI</h2><span>Windows Credential Manager / {h?.deepseek_credentials_configured ? 'configured' : 'missing'}</span></div></div>
        <label>API key<input type="password" autoComplete="off" value={deepseekKey} onChange={(event) => setDeepseekKey(event.target.value)} /></label>
