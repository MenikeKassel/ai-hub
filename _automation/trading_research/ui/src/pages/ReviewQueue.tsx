...
import type { QueueScope, ReviewView } from '../features/reviews/types'
import QueryState from '../components/QueryState'
import { shanghaiDate, updateRoute, useRoute } from '../workspace'

const views: Array<[ReviewView, string]> = [['new', '新增帖子'], ['processed', 'AI 已处理'], ['pending', '待你确认'], ['failed', 'AI 失败'], ['approved', '今日批准']]
...
export default function ReviewQueue() {
  ...
  const reviewDate = /^\d{4}-\d{2}-\d{2}$/.test(params.get('date') || '') ? params.get('date')! : shanghaiDate()
  const scope: QueueScope = params.get('scope') === 'backlog' ? 'backlog' : 'morning'
  const view: ReviewView = views.some(([key]) => key === params.get('view')) ? params.get('view') as ReviewView : 'pending'
  const page = Math.max(1, Number(params.get('page')) || 1)
  const attentionOnly = params.get('attention') === '1'
  const queryParams = new URLSearchParams({ review_date: reviewDate, scope, view, page: String(page), page_size: '50', attention_only: String(attentionOnly) })
  ...
  const bulkPreview = useMutation({ mutationFn: () => api.bulkPreviewRecommendationDrafts(reviewDate, scope), onSuccess: (value) => { setPreview(value); setResult(null) } })
  ...
  const canBulk = scope === 'morning' && reviewDate === shanghaiDate() && view === 'pending' && !attentionOnly
  useEffect(() => { setPreview(null); setResult(null) }, [scope, reviewDate, view, attentionOnly, page])
  const delivery = query.data?.delivery
  const deliveryErrors = delivery?.errors || []
  const switchQueue = (date: string, queue: QueueScope) => updateRoute({ date, scope: queue, page: 1, post: null, detail: null, attention: null })
  const sourceItem = detail.data ? { postId: selectedId, post: detail.data.post, drafts: detail.data.drafts, historical: scope === 'backlog' } : null
  ...
  return <section className={`review-workspace ${params.get('detail') === '1' ? 'show-review-detail' : ''}`}>
    ...
    <div className="review-scope-bar"><h2>{scope === 'backlog' ? '历史待办' : reviewDate === shanghaiDate() ? '今日审核' : '下一晨报'}</h2><nav className="segmented" aria-label="审核范围"><button className={scope === 'morning' && reviewDate === shanghaiDate() ? 'active' : ''} onClick={() => switchQueue(shanghaiDate(), 'morning')}>今日审核</button><button className={scope === 'morning' && reviewDate === shanghaiDate(1) ? 'active' : ''} onClick={() => switchQueue(shanghaiDate(1), 'morning')}>下一晨报</button><button className={scope === 'backlog' ? 'active' : ''} onClick={() => switchQueue(shanghaiDate(), 'backlog')}>历史待办</button></nav><label className="review-date">审核日期<input aria-label="审核日期" type="date" value={reviewDate} onChange={(event) => switchQueue(event.target.value, 'morning')} /></label></div>
    {scope === 'morning' && delivery && <div className={`delivery-strip ${delivery.status}`}><strong>09:00 晨报 · <span className="delivery-status">{deliveryLabels[delivery.status] || delivery.status}</span></strong><span>{delivery.successful_kols} / {delivery.active_kols} 个账号完成采集 · 覆盖率 {delivery.coverage == null ? '未知' : `${Math.round(delivery.coverage * 100)}%`}</span><details><summary>交付详情{deliveryErrors.length ? ` · ${deliveryErrors.length} 项异常` : ''}</summary><p>{delivery.completed_at ? `阶段完成：${formatDate(delivery.completed_at)}` : '阶段尚未完成'}</p>{Object.entries(delivery.platform_breakdown || {}).map(([platform, value]) => <p key={platform}>{platform.toUpperCase()}：成功 {value.success} / 目标 {value.target} · 失败 {value.failed} · 等待 {value.pending}</p>)}{!Object.keys(delivery.platform_breakdown || {}).length && <p>这次记录缺少分平台统计。</p>}{deliveryErrors.map((error, index) => <p key={index}>{error}</p>)}</details></div>}
    <div className="review-counters">{views.map(([key, label]) => <button key={key} aria-pressed={view === key} className={view === key ? 'active' : ''} onClick={() => updateRoute({ view: key, page: 1, post: null, detail: null, attention: null })}><span>{label}</span><strong>{query.data?.counts?.[key] ?? '—'}<small>篇</small></strong></button>)}</div>
    <div className="queue-toolbar"><div><strong>{scope === 'backlog' ? '历史待办' : `${reviewDate} 晨报`}</strong><span>共 {query.data?.total_posts ?? '—'} 篇帖子 · {query.data?.total_drafts ?? '—'} 条草稿</span></div><div className="header-actions">{view === 'pending' && <label className="checkbox-inline"><input type="checkbox" checked={attentionOnly} onChange={(event) => updateRoute({ attention: event.target.checked ? '1' : null, page: 1, post: null })} />只看需核对</label>}{canBulk && <button className="primary-button" disabled={bulkPreview.isPending} onClick={() => bulkPreview.mutate()}>预览批量批准</button>}<button className="icon-button" title="刷新队列" onClick={refresh}><RefreshCw size={16} /></button></div></div>
    ...
    <div className="review-layout compact-review-layout"><aside className="post-list" aria-label="审核帖子列表">{items.map((item) => <button className={`post-item ${item.post_id === selectedId ? 'selected' : ''}`} key={item.post_id} onClick={() => updateRoute({ post: item.post_id, detail: '1' })}><div className="post-item-heading"><strong>{item.display_name || item.handle}</strong><span className="badge neutral">{item.platform}</span></div><small>{formatDate(item.posted_at)}</small><p>{item.excerpt || '图片或长文帖子'}</p><div className="post-item-footer"><span>{item.draft_count} 条草稿</span>{item.attention_count > 0 && <span className="badge amber">{item.attention_count} 条需核对</span>}{item.failure_kind && <span className="badge red">{failureLabels[item.failure_kind]}</span>}</div></button>)}{!query.isLoading && !items.length && <div className="empty-state"><strong>当前范围暂无记录</strong><span>可以切换日期或查看历史待办。</span></div>}</aside><div className="review-detail"><button className="secondary-button detail-back" onClick={() => updateRoute({ detail: null })}><ArrowLeft size={16} />返回列表</button><QueryState loading={detail.isLoading && Boolean(selectedId)} error={detail.error} stale={Boolean(detail.data)} />{sourceItem && <PostDetail key={selectedId} item={sourceItem} reviewDate={reviewDate} onDone={refresh} />}{!selectedId && <div className="empty-state">选择一篇帖子查看原文与草稿</div>}</div></div>
