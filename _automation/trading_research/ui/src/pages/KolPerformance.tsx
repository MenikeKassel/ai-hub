...
function Trend({ points }: { points: Array<{ as_of: string; payload: KolPerformanceRow }> }) {
  const values = points.map((point) => point.payload.metrics.median_excess).filter((value): value is number => value !== null && Number.isFinite(value))
  if (values.length < 2) return <div className="empty-state compact">运行更多检查节点后显示滚动趋势。</div>
  const min = Math.min(...values)
  const max = Math.max(...values)
  const span = max - min || 1
  const coords = points.map((point, index) => {
    const value = point.payload.metrics.median_excess
...
export default function KolPerformance() {
  ...
  return <section className="kol-performance-page">
    <header className="page-header"><div><span className="eyebrow">CAPABILITY & STABILITY</span><h1>KOL表现</h1><p className="page-subtitle">按推荐帖子批次等权统计；指标来自冻结收益，AI解读不参与排名。</p></div><button className="primary-button" onClick={() => refresh.mutate()} disabled={refresh.isPending}><RefreshCw size={16} className={refresh.isPending ? 'spin' : ''} />刷新快照</button></header>
