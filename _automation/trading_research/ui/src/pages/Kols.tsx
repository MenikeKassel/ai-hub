...
export default function Kols() {
  ...
  const backfill = useMutation({ mutationFn: ({ id, count }: { id: number; count: number }) => api.queueKolBackfill(id, count), onSuccess: () => client.invalidateQueries({ queryKey: ['kols'] }) })
  ...
  return <section>
    ...
    {view === 'accounts' && <div className="panel">
      ...
      <tbody>{accountRows.map((kol) => <tr key={kol.id}>
        ...
        <td><span className={`badge ${kol.backfill_status === 'queued' || kol.backfill_status === 'needs_review' ? 'amber' : 'neutral'}`}>{kol.backfill_status === 'queued' ? `已补 ${kol.backfill_completed_depth}/${kol.backfill_requested}` : kol.backfill_status === 'needs_review' ? `需复核 ${kol.backfill_completed_depth}/${kol.backfill_requested}` : kol.backfill_status}</span>{kol.backfill_warning && <span className="secondary-line" title={kol.backfill_warning}>本次补抓未达目标；可用右侧按钮重新排队</span>}</td>
        ...
        <td className="actions">{editingId === kol.id ? <><button className="icon-button" title="保存修改" onClick={() => patch.mutate({ id: kol.id, value: editForm })}><Check size={16} /></button><button className="icon-button" title="取消修改" onClick={() => setEditingId(null)}><X size={16} /></button></> : <><button className="icon-button" title={kol.backfill_status === 'needs_review' ? `重新排队补抓 ${kol.backfill_requested} 条` : '排队补抓最近200条'} disabled={backfill.isPending || kol.backfill_status === 'queued'} onClick={() => backfill.mutate({ id: kol.id, count: kol.backfill_status === 'needs_review' ? kol.backfill_requested : 200 })}><History size={16} /></button><button className="icon-button" title="编辑KOL及可用状态" onClick={() => beginEdit(kol)}><Pencil size={16} /></button><button className="icon-button" title={kol.status === 'active' ? '暂停采集' : '恢复采集'} onClick={() => patch.mutate({ id: kol.id, value: { status: kol.status === 'active' ? 'paused' : 'active' } })}>{kol.status === 'active' ? <Pause size={16} /> : <Play size={16} />}</button></>}</td>
      </tr>)}</tbody></table></div>
